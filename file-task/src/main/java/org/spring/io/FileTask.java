...
@Configuration
@EnableTask
public class FileTask implements CommandLineRunner {

	@Autowired
	private Environment environment;

	@Autowired
	private FlatFileItemReader<Trade> itemReader;

	@Bean
	public FlatFileItemReader<Trade> reader() {
		FlatFileItemReader<Trade> reader = new FlatFileItemReader<Trade>();
		reader.setResource(new FileSystemResource(environment.getProperty("fileName")));
		reader.setLineMapper(new DefaultLineMapper<Trade>() {{
			setLineTokenizer(new DelimitedLineTokenizer() {{
				setNames(new String[] {"symbol", "type", "amount" });
				setDelimiter(" ");
			}});
	...
	@Override public void run(String... args) throws Exception {
		   itemReader.open(new ExecutionContext());
			Trade trade;
			while ((trade = itemReader.read()) != null){
